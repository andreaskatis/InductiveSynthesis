...
\mike{[More here about alternate approaches?]}

In this paper, we propose a new approach that can synthesize programs that do not have to conform to specific template shapes for arbitrary {\em assume/guarantee contracts} containing safety properties involving linear integer arithmetic\mike{is this right?}.  Although the technique is not guaranteed to succeed or terminate, but we have used it to successfully synthesize a range of programs over non-trivial contracts.  \mike{What is our ``hook''?  Why is this *better* than other approaches to synthesis - this (unwritten) sentence is critically important}.

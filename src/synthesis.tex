...
$\mathit{BaseCheck_k}$ ever fails, we know $\mathit{BaseCheck(k)}$
would also fail and so the system is unrealizable. At the same time,
the algorithm tries to prove $\mathit{ExtendCheck(k)}$. As soon as the
inductive step of $\mathit{ExtendCheck(k)}$ passes, we have a complete
...
\For{$(k \gets 0; \mathbf{true}; k \gets k + 1)$}{	
$BaseResult \gets \textsc{\aeval}(BaseCheck_{k})$\;
$ExtendResult \gets \textsc{\aeval}(ExtendCheck(k))$;
\\

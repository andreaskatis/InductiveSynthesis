...
\SetKw{KwContinue}{continue}
\KwIn{Assume-Guarantee Contract in Lustre, $(A,G)$}
\KwOut{Skolem collection \skolems, \hspace{+1cm}Return value
$\in \{\realizable, \unrealizable\}$.
}
\BlankLine
$i \gets 0$;\\
...
\\
\While{}{	
	\uIf(\label{alg:returnUnsat}){(\textsc{BaseEngine}.\isValid($BaseResult$))}
	{
		\Return
		\unrealizable 
	}

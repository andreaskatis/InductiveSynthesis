...
procedure. The algorithm repeatedly proves \textit{BaseCheck(i)} and
accumulates the resulting Skolem functions. If \textit{BaseCheck(i)}
ever fails, the system is unrealizable. At the same time, the
...
corresponds to the inductive step, and return the collection of the
Skolem functions.
...
$BaseResult \gets \textsc{\aeval}(BaseCheck(i))$\;
...
the implementation begins (method \textsc{assign\_$G_{I}$\_witness(fig:syntha)})
...
\andrew{I think the counts are still off in the previous paragraph. If
 the first $k-1$ elements correspond to BaseCheck then why does the
 template run from skolems[0] up to skolems[k-1]. That's $k$ elements.
...
Functions] Let $BaseCheck_{S(s_n,i,s')}(n)$ and
...
\grigory{I suggest to re-run this experiment with guarantee5 (added yesterday).
I'm curious to see the C-implementation.}

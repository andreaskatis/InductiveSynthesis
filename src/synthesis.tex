...
An Assume-Guarantee (AG) contract can be formally defined by two disjoints sets, a set of
...
\andrew{We need to say something about checking the $G_I$ is
  non-empty. We currently don't mention it here or in our algorithm,
  but it must play a cruical role, no?}
...
\BlankLine
$i \gets 0$;\\
\grigory{Something is wrong here. The counter $i$ is incremented in
  the loop, but used only outside of the loop.\\}
\andrew{Also, this algorithm uses the negation of the checks we have
  been describing, but it doesn't explain this or call it out
  anywhere. I think we shouldn't do this anyway. I'd prefer if we say
  baseengine is invalid means unrealizable, etc.}\\
\andrew{Also, Skolems is never initialized}\\
...
\andrew{This numbers are off here. Skolems looks like it has length
  $k+1$ from the algorithm.}
...
\begin{itemize}
\john{I feel like we could be more clear than saying "quantified part has been substituted with a witnessing Skolem function. Maybe say something like: replace the expression ``$\exists s'.~ G_T(s_n, i, s')$'' with ``$S(s_n,i,s')$''}
...
\john{This example doesn't really show a lustre ``contract''. It does not have assumptions nor guarantees? Maybe we could change the variable names from \texttt{prop} to \texttt{guarantee}?}
...
\andrew{We need to double check indicies and lengths in the previous paragraph}

...
\textit{abs(k-1)} in Template~\ref{fig:synthalg}). This is to ensure that we
capture cases where the initial state guarantees $G_I$ diffesr from the
...
correctness, we wrote machine-checked proofs regarding the validity of $\mathit{BaseCheck(n)}$ and
...
\end{theorem}
\begin{proof}
The proof uses the definition $\mathit{Extend_n(s)}$ of an extendable state,
...
Lustre and as an automaton. There are two unassigned variables \texttt{x} and
